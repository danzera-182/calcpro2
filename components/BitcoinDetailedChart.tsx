...
import { BitcoinPriceHistoryPoint } from '../types';
import { fetchBitcoinHistoricalChartData } from '../utils/economicIndicatorsAPI';
import Button from './ui/Button';
import { formatCurrency } from '../utils/formatters';
import { useTheme } from '../hooks/useTheme';

interface BitcoinDetailedChartProps {
  coinId?: string;
  vsCurrency?: string;
}
...
const BitcoinDetailedChart: React.FC<BitcoinDetailedChartProps> = ({
  coinId = 'bitcoin',
  vsCurrency = 'usd',
}) => {
  ...
  const loadChartData = useCallback(async (period: ChartPeriod) => {
    ...
    try {
      ...
      const data = await fetchBitcoinHistoricalChartData(coinId, vsCurrency, days);
      ...
      console.error('Error fetching Bitcoin historical data:', e);
  ...
  const lineStrokeColor = theme === 'dark' ? '#F9AA4B' : '#F7931A';
      ...
      <ResponsiveContainer width="100%" height={350}>
        <LineChart data={chartData} margin={{ top: 5, right: 20, left: 25, bottom: 5 }}>
          ...
          <Line
            type="monotone"
            dataKey="price"
            name={`Bitcoin (${vsCurrency.toUpperCase()})`}
            stroke={lineStrokeColor}
            strokeWidth={2}
            dot={chartData.length < 100 ? { r: 2, fill: lineStrokeColor } : false}
            activeDot={{ r: 5 }}
            isAnimationActive={!isLoading} // Only animate if not loading initially
          />
        </LineChart>
      </ResponsiveContainer>
    );
...
export default BitcoinDetailedChart;